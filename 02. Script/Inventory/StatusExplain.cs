...
public class StatusExplain : MonoBehaviour
{
    ...
    public void SetHpExplain(float _maxHp, float _hp)
    {
        ...
        statusExplain.text = (GameManager.language == Language.Ko) ?
            $"<size=+4>{_maxHp:F0}</size>의 체력 중\n<size=+4>{_hp:F0}</size>이 남았습니다." :
            $"Out of <size=+4>{_maxHp:F0}</size> health points,\n<size=+4>{_hp:F0}</size> are remaining.";
    }
    ...
    public void SetAbilityExplain(float _ability)
    {
        ...
        statusExplain.text = (GameManager.language == Language.Ko) ?
            $"스킬과 기본공격이\n<size=+4>{_ability:F0}</size>만큼 강해집니다." :
            $"Skills and default attack\nbecome <size=+4>{_ability:F0}</size> stronger.";
    }
    ...
    public void SetResistExplain(float _resist)
    {
        ...
        statusExplain.text = (GameManager.language == Language.Ko) ?
            $"받는 피해가 <size=+4>{_resist:F1}%</size>\n만큼 감소합니다." :
            $"The damage taken is\nreduced by <size=+4>{_resist:F1}%</size>";
    }
    ...
    public void SetSpeedExplain(float _speed)
    {
        ...
        statusExplain.text = (GameManager.language == Language.Ko) ?
            $"시전 속도가 <size=+4>{_speed:F1}%</size>\n빨라집니다." :
            $"Casting speed\nincreases by <size=+4>{_speed:F1}%</size>";
    }
